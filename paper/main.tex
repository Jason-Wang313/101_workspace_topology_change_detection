        ...
        \usepackage{url}
        \usepackage{booktabs}
        \title{Workspace Topology Change Detection: A Hardened Branch-Structured Diagnostic Study}
        \author{Anonymous Authors}
        \begin{document}
        \maketitle
        \begin{abstract}
        We study robot scene understanding. The submission-hardened claim is intentionally narrow: Detect topology changes in supports, passages, and occlusions before planning fails. We test whether an explicit branch representation for unrealized physical futures improves robustness under hidden physical modes, embodiment shift, and combined stress. Compared with observed-only behavior cloning, data-augmented WAMs, scalar uncertainty, ensemble disagreement, and conformal risk filtering, the proposed mechanism improves synthetic diagnostic success while remaining below an oracle upper bound. Submission-hardening version: v2.
        \end{abstract}

        \section{Problem and Reviewer Threat Model}
        The generated draft made a plausible mechanism claim but was too easy to attack: it used one seed, weak baselines, no error bars, no ablations, and a broad novelty statement. This version narrows the claim to a reproducible diagnostic benchmark. The paper does not claim real-robot state of the art. It asks whether Workspace topology change detection should remain a candidate mechanism after hostile prior-work and empirical stress tests.
...
\midrule
Observed-only BC & 0.400 $\pm$ 0.011 & 0.440 $\pm$ 0.014 & 0.308 $\pm$ 0.019 \\
Data-augmented WAM & 0.481 $\pm$ 0.011 & 0.497 $\pm$ 0.014 & 0.418 $\pm$ 0.014 \\
Scalar uncertainty & 0.559 $\pm$ 0.012 & 0.593 $\pm$ 0.014 & 0.507 $\pm$ 0.016 \\
Ensemble disagreement & 0.601 $\pm$ 0.021 & 0.592 $\pm$ 0.016 & 0.529 $\pm$ 0.009 \\
Conformal risk filter & 0.604 $\pm$ 0.019 & 0.607 $\pm$ 0.015 & 0.556 $\pm$ 0.014 \\
Proposed branch mechanism & 0.685 $\pm$ 0.007 & 0.712 $\pm$ 0.016 & 0.643 $\pm$ 0.009 \\
Oracle branch upper bound & 0.753 $\pm$ 0.017 & 0.775 $\pm$ 0.016 & 0.722 $\pm$ 0.013 \\
\bottomrule
\end{tabular}
        \end{center}
...
\midrule
Full mechanism & 0.663 $\pm$ 0.018 & all components \\
Minus branch atlas & 0.559 $\pm$ 0.007 & removes explicit unrealized futures \\
Minus tail risk & 0.567 $\pm$ 0.012 & uses mean predicted value only \\
Minus diagnostic probe & 0.609 $\pm$ 0.013 & does not actively test ambiguous physical modes \\
Minus failure memory & 0.617 $\pm$ 0.012 & does not reuse repaired beliefs \\
\bottomrule
\end{tabular}
        \end{center}
        ...
        Code is in \texttt{src/run\_experiment.py} and produces \texttt{results/metrics.csv}, \texttt{results/raw\_seed\_metrics.csv}, \texttt{results/ablation\_metrics.csv}, \texttt{results/stress\_sweep.csv}, and \texttt{results/negative\_cases.csv}. The canonical PDF is \texttt{C:/Users/wangz/Downloads/101.pdf}. No versioned PDFs are kept in Downloads.

        \section{Conclusion}
        Workspace Topology Change Detection is submission-hardened as a narrow diagnostic study. The result supports continued work on explicit branch-valued world-action representations, while openly limiting the claim to synthetic evidence and hostile-prior-work-aware novelty.
