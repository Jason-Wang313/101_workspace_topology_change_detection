        ...
        \usepackage{url}
        \usepackage{booktabs}
        \title{Workspace Topology Change Detection: Branch-Structured Evidence for Embodied World-Action Models}
        \author{Anonymous Authors}
        \begin{document}
        \maketitle
        \begin{abstract}
        We study robot scene understanding. The motivating bet is: Detect topology changes in supports, passages, and occlusions before planning fails.. We introduce a mechanism-level representation that keeps physically admissible but unrealized action outcomes as explicit branches. A synthetic contact-style evaluation shows that the proposed branch mechanism improves hidden-mode success and tail-risk relative to observed-only, augmentation-as-data, and scalar-uncertainty baselines.
        \end{abstract}

        \section{Introduction}
        Robotics papers often collapse unobserved physical alternatives into extra data, variance, or a single predicted next state. For robot scene understanding, this can erase the difference between modes that are visually similar but action-critical. This paper asks whether a world-action model should expose those alternatives to the planner before execution.
...
augmentation\_as\_data\_wam & nominal & 0.630 & 0.370 \\
augmentation\_as\_data\_wam & hidden\_mode\_shift & 0.245 & 0.755 \\
scalar\_uncertainty\_wam & nominal & 0.637 & 0.363 \\
scalar\_uncertainty\_wam & hidden\_mode\_shift & 0.352 & 0.647 \\
proposed\_branch\_mechanism & nominal & 0.760 & 0.240 \\
...
\end{tabular}
        ...
        \section{Conclusion}
        Workspace Topology Change Detection argues that embodied world-action models should preserve unrealized physical alternatives when those alternatives change downstream action value.
